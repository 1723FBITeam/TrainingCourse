---
title: "Unit 3: Trace a Sensor Reading"
...
- Documented the sensor trace using the same step-by-step format from Unit 2

In Unit 2 you traced *forward* — from a button press to a motor output. This time you'll trace *backward* — starting from a sensor (the Limelight camera) and following the data through the code to see where and how it's used.

:::tip[Before You Start]
Make sure you've completed **Unit 2: Trace a Button Press** and **Lesson 5: RobotContainer & Bindings**. Have the robot project open in your IDE so you can follow along in the actual code.
:::
...
Add this sensor trace to your **Trace Worksheet** deliverable. Use the same table format from Unit 2:
...
You now have two trace techniques. **Forward traces** (Unit 2) start from a human action and follow it to a motor output. **Reverse traces** (this unit) start from a sensor input and follow it to where the data is used. Both are essential for understanding robot code — use whichever direction makes sense for the question you're trying to answer.
:::
...
In **Unit 6: Autonomous**, you'll see how the corrected pose estimate is used during autonomous routines to follow pre-planned paths accurately.
