---
title: "Unit 2: Trace a Button Press"
...
- Documented your trace using the step-by-step format from Lesson 5
...
Make sure you've completed **Lesson 5: RobotContainer & Bindings**. You saw a full trace of the Y button (shoot) there. Now you'll do the same thing yourself for the X button (intake). Have the robot project open in your IDE so you can follow along in the actual code.
:::
...
Remember from Lesson 5: `toggleOnTrue` means the button acts like a light switch. Press X once → the command starts. Press X again → the command is cancelled (and cleanup runs). The driver doesn't have to hold the button.
...
For your **Trace Worksheet** deliverable, write down the trace you just completed. Use this format for each step:
...
Save this — you'll submit it as part of the Trace Worksheet deliverable.
...
In **Unit 3: Trace a Sensor**, you'll trace in the other direction — from a sensor reading back through the code to see how the robot uses that data to make decisions.
