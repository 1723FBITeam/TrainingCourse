...
Make sure you've completed **Lesson 2: Robot Lifecycle** so you understand how the robot starts up and runs its periodic loop. Subsystems plug directly into that lifecycle — their `periodic()` method gets called every loop cycle.
:::
...
Now that you understand what subsystems are and how they control hardware, it's time to learn about **commands** — the code that tells subsystems *when* and *how* to act. In **Lesson 4: Commands**, you'll see how `AutoShootCommand` coordinates the turret, flywheels, and feeder to score a note.
