...
export function GitHubLink({ path, startLine, endLine, label, isFolder }: GitHubLinkProps) {
  const displayLabel = label ?? path.split('/').filter(Boolean).pop() ?? path;

  if (!robotConfig.repoUrl) {
    return <span className="github-link-fallback">{displayLabel}</span>;
  }

  const segment = isFolder ? 'tree' : 'blob';
  let url = `${robotConfig.repoUrl}/${segment}/${robotConfig.defaultBranch}/${path}`;

  if (startLine != null) {
    url += endLine != null ? `#L${startLine}-L${endLine}` : `#L${startLine}`;
  }
